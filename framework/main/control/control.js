...
accountCurrent = new Account("CURRENT", "AAA");

viewer = new ContentViewer(contentSpace, algo, accountCurrent);
...
post0.addComment(account0, "", new DataComment(account0, "so hot omg", algo.getNextCommentId()));
algo.add(post0);

post1 = new DataContentMain(account1, _localEscape + "media/clips/fortnite_1.mp4", "40k", "🍆🍑💚", "title", "description");
post1.addComment(account1, "", new DataComment(account1, "epic", algo.getNextCommentId()));
algo.add(post1);

post2 = new DataContentMain(account0, _localEscape + "media/clips/counter_strike_2_3.mp4", "237", "🍆🍑💚", "title", "description");
post2.addComment(account0, "", new DataComment(account0, "yo too sick ngl", algo.getNextCommentId()));
algo.add(post2);

post3 = new DataContentMain(account0, _localEscape + "media/clips/counter_strike_2_7.mp4", "521M", "🍆🍑💚", "title", "description");
post3.addComment(account0, "", new DataComment(account0, "yo you have a wife? impressive feat for a gamer", algo.getNextCommentId()));
algo.add(post3);

post4 = new DataContentMain(account1, _localEscape + "media/clips/fortnite_3.mp4", "69 trillion", "🍆🍑💚", "title", "description");
post4.addComment(account1, "", new DataComment(account1, "based opinion mate, what are your thoughts on current issues", algo.getNextCommentId()));
algo.add(post4);
